...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    # Reserve the runnable parallel batch atomically. Calls outside the hard
    # budget remain in transcript order as blocked results and never execute.
    for index, call in enumerate(parsed_calls):
        if call[4] is not None:
            continue
        tc, name, args, trace, _block, scope_block = call
        file_block = _apply_cron_file_preflight(
            agent, name, args, count_reads=False
        )
        if file_block is not None:
            parsed_calls[index] = (tc, name, args, trace, file_block, scope_block)

    runnable_indices = [
        index for index, call in enumerate(parsed_calls) if call[4] is None
    ]
    ...
    if granted < len(runnable_indices):
        ...
        for index in runnable_indices[granted:]:
            tc, name, args, trace, _block, scope_block = parsed_calls[index]
            parsed_calls[index] = (
                tc,
                name,
                args,
    ...
    for index in runnable_indices[:granted]:
        tc, name, args, trace, block, scope_block = parsed_calls[index]
        if block is not None:
            continue
        file_block = _apply_cron_file_preflight(agent, name, args)
        if file_block is not None:
            parsed_calls[index] = (
                tc, name, args, trace, file_block, scope_block
            )
...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if _ts_scope_block is None:
            _cron_block = _apply_cron_file_preflight(
                agent, function_name, function_args, count_reads=False
            )
            if _cron_block is None and _reserve_cron_tool_calls(agent, 1) != 1:
                _cron_block = _cron_guardrail_error(
                    "max_tool_calls",
                    f"total tool-call limit {getattr(agent, 'max_tool_calls', 0)} reached",
                )
            if _cron_block is None:
                _cron_block = _apply_cron_file_preflight(
                    agent, function_name, function_args
                )
            if _cron_block is not None:
                try:
                    _ts_scope_block = json.loads(_cron_block)["error"]
                except Exception:
                    _ts_scope_block = _cron_block
