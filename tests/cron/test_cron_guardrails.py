...
class TestToolCallAndFileGuardrails:
    ...
    def test_file_posture_filters_live_same_toolset_plugin_from_schema(self, tmp_path):
        ...
        try:
            ...
            # Current upstream snapshots built-in toolset membership at module
            # discovery time, so a late registry addition is no longer
            # re-expanded by get_tool_definitions(). Model the already-built
            # agent surface directly; this test's contract is the immutable
            # exact-name clamp, independent of registry cache semantics.
            tools.append({
                "type": "function",
                "function": {
                    "name": "host_exec",
