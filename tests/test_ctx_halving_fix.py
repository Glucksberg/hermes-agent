...
class TestParseTpmRequestLimit:
    """OpenAI TPM request-size errors are fixed by reducing the request."""

    def _parse(self, msg):
        from agent.model_metadata import parse_tpm_request_limit_from_error
        return parse_tpm_request_limit_from_error(msg)

    def test_openai_request_too_large_extracts_limit(self):
        msg = (
            "Request too large for gpt-4o on tokens per min (TPM): "
            "Limit 30000, Requested 30218. The input or output tokens must be reduced."
        )
        assert self._parse(msg) == 30000

    def test_comma_separated_numbers(self):
        msg = "tokens per min (TPM): Limit 30,000, Requested 30,218"
        assert self._parse(msg) == 30000

    def test_transient_tpm_rate_limit_is_not_request_too_large(self):
        msg = (
            "Rate limit reached for gpt-4o on tokens per min (TPM): "
            "Limit 30000, Used 11463, Requested 29507."
        )
        assert self._parse(msg) is None

    def test_non_tpm_limit_is_ignored(self):
        msg = "context length exceeded: limit 30000, requested 30218"
        assert self._parse(msg) is None
