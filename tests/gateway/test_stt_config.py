...
@pytest.mark.asyncio
async def test_direct_audio_reply_does_not_transcribe_unrelated_observed_audio(tmp_path):
    from gateway.run import GatewayRunner

    direct_audio = tmp_path / "direct-reply.ogg"
    older_observed_audio = tmp_path / "older-observed.ogg"
    direct_audio.write_bytes(b"direct ogg bytes")
    older_observed_audio.write_bytes(b"older observed ogg bytes")
    ...
    replied_audio_note = (
        f"[Replied-to audio 'direct-reply.ogg' saved at: {direct_audio}]"
    )
    event = MessageEvent(
        text="o que tem nesse audio?",
        message_type=MessageType.VOICE,
        source=source,
        media_urls=[str(direct_audio)],
        media_types=["audio/ogg"],
        reply_to_message_id="direct-audio-message",
        reply_to_text=replied_audio_note,
    ...
    history = [
        {
            ...
            "observed": True,
            "content": (
                f"[Alice|111]\n[audio 'older-observed.ogg' saved at: "
                f"{older_observed_audio}]"
            ),
        }
    ]
    ...
    def _transcribe(path):
        transcript = (
            "direct reply transcript"
            if Path(path).name == "direct-reply.ogg"
            else "unrelated older observed transcript"
        )
        return {"success": True, "transcript": transcript, "provider": "openai"}

    with patch(
        "gateway.run._resolve_observed_audio_cache_path",
        return_value=str(older_observed_audio.resolve()),
    ), patch(
        "tools.transcription_tools.transcribe_audio",
        side_effect=_transcribe,
    ) as transcribe:
        prepared_message = await runner._prepare_inbound_message_text(
            event=event,
            source=source,
            history=history,
        )
        enriched_history = await runner._enrich_observed_audio_context(
            history,
            channel_prompt=event.channel_prompt,
            current_message=event.text,
            explicit_audio_reference=event.reply_to_text,
        )

    assert [Path(call.args[0]).name for call in transcribe.call_args_list] == [
        "direct-reply.ogg"
    ]
    assert "direct reply transcript" in prepared_message
    assert enriched_history is history
    assert "unrelated older observed transcript" not in str(enriched_history)
...
@pytest.mark.asyncio
async def test_observed_audio_transcript_cache_is_bounded_lru(tmp_path):
    from gateway.run import (
        GatewayRunner,
        _OBSERVED_AUDIO_TRANSCRIPT_CACHE_MAX_SIZE,
    )

    runner = GatewayRunner.__new__(GatewayRunner)
    runner.config = GatewayConfig(stt_enabled=True)
    audio_paths = []
    for index in range(_OBSERVED_AUDIO_TRANSCRIPT_CACHE_MAX_SIZE + 1):
        audio_path = tmp_path / f"audio-{index:03d}.ogg"
        audio_path.write_bytes(f"audio-{index:03d}".encode())
        ...
        history = [
            {
                "role": "user",
                "observed": True,
                "content": f"[Alice|111]\n[audio '{audio_path.name}' saved at: {audio_path}]",
            }
        ]
        return await runner._enrich_observed_audio_context(
            history,
            channel_prompt="observed Telegram group context",
            current_message="transcreva o último áudio",
        )
    ...
    with patch(
        ...
        side_effect=lambda path: {
            "success": True,
            "transcript": f"transcript-{Path(path).stem}",
            "provider": "openai",
        },
    ) as transcribe:
        ...
        # A hit must avoid STT and make the oldest entry most recently used.
        await enrich(audio_paths[0])
        assert transcribe.call_count == _OBSERVED_AUDIO_TRANSCRIPT_CACHE_MAX_SIZE

        await enrich(audio_paths[-1])
        cache = runner._observed_audio_transcript_cache
        cached_paths = {key[0] for key in cache}
        assert len(cache) == _OBSERVED_AUDIO_TRANSCRIPT_CACHE_MAX_SIZE
        assert str(audio_paths[0].resolve()) in cached_paths
        assert str(audio_paths[1].resolve()) not in cached_paths

        # The evicted entry retranscribes if requested again.
        await enrich(audio_paths[1])
        assert transcribe.call_count == _OBSERVED_AUDIO_TRANSCRIPT_CACHE_MAX_SIZE + 2
