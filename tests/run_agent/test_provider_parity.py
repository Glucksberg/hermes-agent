...
class TestBuildApiKwargsCodex:
    ...
    def test_direct_openai_gpt4o_omits_reasoning_state(self, monkeypatch):
        agent = _make_agent(monkeypatch, "openai-api", api_mode="codex_responses",
        ...
        assert not any(item.get("type") == "reasoning" for item in kwargs["input"])

    def test_direct_openai_gpt5_keeps_reasoning_state(self, monkeypatch):
        agent = _make_agent(monkeypatch, "openai-api", api_mode="codex_responses",
        ...
        assert kwargs["reasoning"]["effort"] == "medium"
        assert "reasoning.encrypted_content" in kwargs.get("include", [])
        reasoning_items = [item for item in kwargs["input"] if item.get("type") == "reasoning"]
