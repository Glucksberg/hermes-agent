...
def _await_cron_agent(
    *,
    ...
    def _interrupt_and_acknowledge(
        ...
        try:
            request_hard_interrupt(agent, interrupt_reason or message)
        except Exception:
            logger.warning(
    ...
    while True:
        ...
        if duration_limit is not None and elapsed >= duration_limit:
            _interrupt_and_acknowledge(
                f"Cron job '{job_name}' exceeded maximum duration of {duration_limit:g}s"
            )
...
def run_job(
    ...
    try:
        ...
        # Resolve global/per-model reasoning only after provider authentication
        # so an auth fallback can first replace the primary model. An explicit
        # per-job value remains authoritative, including YAML boolean False.
        ...
        # An explicit per-job guardrail overrides the global turn budget.
        ...
        max_iterations = (
            ...
            else (_agent_max_turns or _legacy_max_turns or 500)
        )
        ...
        reasoning_config = (
            parse_reasoning_effort(job["reasoning_effort"])
        ...
        # Preserve scheduler-scoped ContextVar state when the run hops into the
        # worker. The task id pins file/terminal operations to this guarded run.
...
"""
        ...
        if _cron_file_env is not None or _cron_sandbox_env is not None:
            ...
            try:
                with _env_lock:
                    registered = _active_environments.get(_cron_session_id)
                    if any(
            ...
            seen_envs = set()
            for cron_env in (_cron_file_env, _cron_sandbox_env):
                if cron_env is None or id(cron_env) in seen_envs:
                    continue
                seen_envs.add(id(cron_env))
                try:
                    cron_env.cleanup()
                except (Exception, KeyboardInterrupt) as e:
                    logger.debug(
