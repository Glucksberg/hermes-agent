...
class TestClassifyApiError:
    ...
    def test_openai_tpm_request_too_large_is_context_overflow(self):
        """A single request over the TPM cap needs compression, not backoff."""
        msg = (
            "Error code: 400 - {'error': {'message': 'Request too large for "
            "gpt-4o on tokens per min (TPM): Limit 30000, Requested 30218. "
            "The input or output tokens must be reduced.'}}"
        )
        e = MockAPIError(msg, status_code=400)
        result = classify_api_error(e, provider="openai-api", model="gpt-4o")
        assert result.reason == FailoverReason.context_overflow
        assert result.retryable is True
        assert result.should_compress is True

    def test_openai_tpm_transient_rate_limit_stays_rate_limit(self):
        msg = (
