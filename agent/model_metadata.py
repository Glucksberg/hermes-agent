...
def parse_tpm_request_limit_from_error(error_msg: str) -> Optional[int]:
    """Return the TPM/request limit when a single OpenAI request exceeds it.

    OpenAI can reject a request before generation with wording like:
      "Request too large ... tokens per min (TPM): Limit 30000, Requested 30218"

    This is different from a transient rate limit where the current minute is
    exhausted, e.g. "Limit 30000, Used 11463, Requested 29507".  Waiting may
    fix the latter; only reducing the request can fix the former.  Return the
    limit only when the request itself is larger than the per-minute cap.
    """
    error_lower = error_msg.lower()
    if "tokens per min" not in error_lower and "tpm" not in error_lower:
        return None

    def _parse_int(raw: str) -> int:
        return int(raw.replace(",", ""))

    limit_match = re.search(r"\blimit[:\s]+([0-9][0-9,]*)", error_lower)
    requested_match = re.search(r"\brequested[:\s]+([0-9][0-9,]*)", error_lower)
    if not limit_match or not requested_match:
        return None

    limit = _parse_int(limit_match.group(1))
    requested = _parse_int(requested_match.group(1))
    if 1 <= limit < requested:
        return limit
    return None
