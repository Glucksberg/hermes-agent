...
def classify_api_error(
    ...
    *,
    ...
    # OpenAI may return HTTP 400/429 for a request whose token count exceeds
    # the account's tokens-per-minute cap:
    # "Request too large ... tokens per min (TPM): Limit 30000, Requested 30218".
    # That won't recover by waiting or rotating credentials; the request must
    # be compressed/reduced.  Plain transient TPM errors where Requested <=
    # Limit still fall through to rate_limit below.
    if parse_tpm_request_limit_from_error(error_msg) is not None:
        return _result(
