...
def test_replied_to_voice_is_cached_as_audio_context():
    ...
    async def _run():
        ...
        with patch(
            ...
            enriched = await adapter._cache_replied_audio_media(msg, event)

        cache_media.assert_called_once()
        assert enriched.reply_to_text == "[audio 'voice.ogg' saved at: /home/dev/.hermes/audio_cache/audio_reply.ogg]"
        assert enriched.media_urls == ["/home/dev/.hermes/audio_cache/audio_reply.ogg"]
        assert enriched.media_types == ["audio/ogg"]
