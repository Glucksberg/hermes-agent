...
class _CapturingAgent:
    ...
    def run_conversation(self, user_message: str, conversation_history=None, task_id=None):
        type(self).last_message = user_message
        type(self).last_history = conversation_history
        return {
            "final_response": "ok",
            "messages": [],
            "api_calls": 1,
...
def test_telegram_topic_model_override_routes_specific_topic(monkeypatch):
    import hermes_cli.runtime_provider as runtime_provider
    ...
    def fake_resolve_runtime_provider(
        *,
        ...
        return {
            "api_key": explicit_api_key or "sk-openai",
    ...
    assert model == "gpt-4o"
    assert runtime_kwargs["provider"] == "openai-api"
    ...
    assert runtime_kwargs["context_length"] == 128000
    assert runtime_kwargs["max_tokens"] == 512
    assert calls == [
        {
    ...
    assert route["runtime"]["context_length"] == 128000
    assert route["runtime"]["max_tokens"] == 512


def test_session_info_uses_telegram_topic_model_override(monkeypatch):
    import hermes_cli.runtime_provider as runtime_provider
    ...
    assert "openai-api" in info
    assert "128K" in info
    assert "gpt-5.5" not in info
    assert "openai-codex" not in info


def test_run_agent_context_includes_topic_runtime_info(monkeypatch):
    import hermes_cli.runtime_provider as runtime_provider
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {
            "api_key": "sk-openai",
            "base_url": kwargs.get("explicit_base_url") or "https://api.openai.com/v1",
            "provider": kwargs.get("requested") or "openai-api",
            "api_mode": "chat_completions",
            "command": None,
            "args": [],
            "credential_pool": None,
    ...
    _CapturingAgent.last_init = None
    _CapturingAgent.last_message = None
    _CapturingAgent.last_history = None
    ...
    result = asyncio.run(
        runner._run_agent(
            message="qual modelo esta rodando agora?",
    ...
    assert result["final_response"] == "ok"
    system_prompt = _CapturingAgent.last_init["ephemeral_system_prompt"]
    assert "## Current Model Runtime" in system_prompt
    assert "Model: `gpt-4o`" in system_prompt
    assert "Provider: openai-api" in system_prompt
    assert "Context: 128K tokens (config)" in system_prompt


def test_telegram_topic_model_override_does_not_affect_other_topics(monkeypatch):
    monkeypatch.setattr(
        gateway_run,
        "_resolve_runtime_agent_kwargs",
        lambda: {
...
def test_telegram_topic_model_override_supports_nested_chat_thread(monkeypatch):
    import hermes_cli.runtime_provider as runtime_provider
