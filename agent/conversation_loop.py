...
from agent.model_metadata import (
    ...
    parse_tpm_request_limit_from_error,
    save_context_length,
)
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if is_context_length_error:
                    ...
                    # Error is about the INPUT being too large — reduce the
                    # operational context budget and compress.  TPM limits are
                    # account/request budgets, not model context windows, so
                    # they may guide this retry but must not be persisted as
                    # model metadata.
                    tpm_request_limit = parse_tpm_request_limit_from_error(error_msg)
                    parsed_limit = None if tpm_request_limit else parse_context_limit_from_error(error_msg)
                    ...
                    if tpm_request_limit and tpm_request_limit < old_ctx:
                        new_ctx = tpm_request_limit
                        agent._vprint(
                            f"{agent.log_prefix}TPM request limit detected from API: "
                            f"{new_ctx:,} tokens (was {old_ctx:,}); compressing without "
                            "persisting model context.",
                            force=True,
                        )
                    elif parsed_limit and parsed_limit < old_ctx:
                    ...
                    if new_ctx and new_ctx < old_ctx:
                        ...
                        if hasattr(compressor, "_context_probed"):
                            ...
                            compressor._context_probe_persistable = bool(
                                parsed_limit and parsed_limit == new_ctx and not tpm_request_limit
                            )
